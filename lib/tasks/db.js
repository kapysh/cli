...
module.exports = {
  "db:migrate": {
    descriptions: {
      ...
      options: {
        "--times": "Defines the number of times to run migration. " +
          clc.blueBright("Default: all")
      }
    },
    ...
    task: function() {
      getMigrator(function(migrator) {
        var options = {
          method: 'up',
          times: args.times || 'all'
        };
        migrator.migrate(options, function(err, result) {
          process.exit(0);
        });
      });
    }
